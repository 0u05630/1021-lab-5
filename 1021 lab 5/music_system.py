# Done by Chan, Yat Hei. SID: 21063861
...
def transpose():
    ...
    for i in range(len(music_data)):
        ...
        if music_data[i][1] < 21:
            music_data[i][1] = 21
        elif music_data[i][1] > 108:
            music_data[i][1] = 108
    #
    #####
...
def adjustSpeed():
    ...
    for i in range(len(music_data)):
        music_data[i][0] = music_data[i][0] * (100/speedchange)
        music_data[i][2] = music_data[i][2] * (100/speedchange)        
    #
    #####
...
def special1():
    ...
    if len(music_data) == 0:
        new_note = [0.0, note_pitch , note_duration]
        music_data.append(new_note)
    else:
        note_time = music_data[-1][0] + music_data[-1][2]
        new_note = [note_time, note_pitch , note_duration]
        music_data.append(new_note)
...
def special2():
    ...
    if end_pitch < start_pitch :
        notes = range(start_pitch, end_pitch - 1, -1)
    elif start_pitch < end_pitch:
        notes = range(start_pitch, end_pitch + 1)
    else:
        notes = start_pitch
    num_of_note = len(notes)
    
    for _ in range(num_times):
        for i in range(len(notes)):
            if len(music_data) == 0:
                new_note = [0.0, notes[i], sequence_duration/num_of_note]
                music_data.append(new_note)
            else:
                new_note = [music_data[-1][0] + music_data[-1][2], notes[i], sequence_duration/num_of_note]
                music_data.append(new_note)
